...
#             guassian_dpt, gaussian_lowres_dpt = False, True
#             if self.config["train_head_only"]:
#                 guassian_dpt, gaussian_lowres_dpt = False, False
#         else:
#             guassian_dpt, gaussian_lowres_dpt = True, False

#         self.encoder.downstream_head1.gaussian_dpt.dpt.requires_grad_(guassian_dpt)
#         self.encoder.downstream_head2.gaussian_dpt.dpt.requires_grad_(guassian_dpt)
#         self.encoder.downstream_head1.gaussian_dpt_lowres.dpt.requires_grad_(gaussian_lowres_dpt)
#         self.encoder.downstream_head2.gaussian_dpt_lowres.dpt.requires_grad_(gaussian_lowres_dpt)
#         if self.config["train_head_only"]:
...
#             th_rgb = config.th_rgb
#             th_depth = config.th_depth
...
#         if config.train_coarse or config.use_lod:
#             # Calculate losses
#             mask = loss_mask.calculate_loss_mask(batch)
...
#         if apply_mask:
#             if not (mask.sum() > 0): print(f"Warning: mask has no valid pixels! batch['scene_id'] = {batch['scene']}")
#             if mask.sum() <= 0:
...
#         num_gaussians = pred1['means'].shape[1] + pred2['means'].shape[1]
