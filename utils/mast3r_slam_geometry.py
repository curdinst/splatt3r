...
def depth_map_to_points(img_size, depthmap, K):
    # depthmap: (b, h, w, 1)
    depthmap = depthmap.unsqueeze(-1)  # Ensure depthmap is (b, h, w, 1)
    uv = get_pixel_coords(depthmap.shape[0], img_size, device=depthmap.device, dtype=depthmap.dtype).view(
        *depthmap.shape[:-1], 2
    )
    Xs = backproject(uv, depthmap, K)
    return Xs
