...
class SaveBatchData(L.Callback):
    ...
    def __init__(self, save_dir, train_save_interval=100, val_save_interval=100, test_save_interval=100, coarse=True, grad_coarseness=False, train_coarse_prediction=False, coarseness_predictions=False, penalty_optimisation=False, config=None):
        ...
        self.penalty_optimisation = penalty_optimisation
        self.config = config
    ...
    def save_batch_data(self, prefix, trainer, pl_module, batch, batch_idx):
        ...
        if self.train_coarse_prediction:
            ...
            # save_as_ply(pred1_coarseness, pred2_coarseness, os.path.join(save_dir, f"gaussians.ply"), grad_coarseness=False)
        ...
        elif self.penalty_optimisation:
             # Run the batch through the model again
            _, _, h, w = batch["context"][0]["img"].shape
            view1, view2 = batch['context']
            target_views = batch['target']
            with torch.no_grad():
                pred1_512, pred2_512, pred1_256, pred2_256, pred1_128, pred2_128, coarseness1, coarseness2 = pl_module.forward(view1, view2)
                color_512, color_512_context = pl_module.decoder(batch, pred1_512, pred2_512, (h, w), render_context=True) # gets (b, v, c, h, w)
                color_256, color_256_context = pl_module.decoder(batch, pred1_256, pred2_256, (h, w), render_context=True) # gets (b, v, c, h, w)
                color_128, color_128_context = pl_module.decoder(batch, pred1_128, pred2_128, (h, w), render_context=True) # gets (b, v, c, h, w)
            # Calculate losses
            mask = loss_mask.calculate_loss_mask(batch)
            ...
            coarseness1, coarseness2 = pl_module.get_gt_coarseness(batch, colors_context, mask=depth_masks)
            ...
            pred1_combined, pred2_combined = pl_module.apply_coarseness_mask(pred1, pred2)
            colors_combined, _ = pl_module.decoder(batch, pred1_combined, pred2_combined, (h, w), fused_gaussians=True)
            colors = (color_512, color_256, color_128, color_512_context, color_256_context, color_128_context)
            ...
            log_batch_files_penalty_optimisation(batch, colors_combined, mask, view1, view2, pred1_combined, pred2_combined, coarseness1, coarseness2, save_dir, colors=colors, grad_coarseness=self.grad_coarseness, pl_module=pl_module)

        elif self.config.train_on_single_prediction:
            # Run the batch through the model again
            _, _, h, w = batch["context"][0]["img"].shape
            view1, view2 = batch['context']
            with torch.no_grad():
                pred1_512, pred2_512, pred1_256, pred2_256, pred1_128, pred2_128 = pl_module.forward(view1, view2)
                # if self.coarse or self.grad_coarseness:
                #     pred1, pred2 = pred1_lowres, pred2_lowres
                
                color_512_pred1, depth_512 = pl_module.decoder(batch, pred1_512, pred2_512, (h, w), fused_gaussians=self.grad_coarseness, use_pred=1)
                color_256_pred1, depth_256 = pl_module.decoder(batch, pred1_256, pred2_256, (h, w), fused_gaussians=self.grad_coarseness, use_pred=1)
                color_128_pred1, depth_128 = pl_module.decoder(batch, pred1_128, pred2_128, (h, w), fused_gaussians=self.grad_coarseness, use_pred=1)

                color_512_pred2, depth_512 = pl_module.decoder(batch, pred1_512, pred2_512, (h, w), fused_gaussians=self.grad_coarseness, use_pred=2)
                color_256_pred2, depth_256 = pl_module.decoder(batch, pred1_256, pred2_256, (h, w), fused_gaussians=self.grad_coarseness, use_pred=2)
                color_128_pred2, depth_128 = pl_module.decoder(batch, pred1_128, pred2_128, (h, w), fused_gaussians=self.grad_coarseness, use_pred=2)
            
            mask1 = loss_mask.calculate_loss_mask(batch, use_context=1)
            mask2 = loss_mask.calculate_loss_mask(batch, use_context=2)
            ...
            log_batch_files(batch, color_512_pred1, depth_512, mask1, view1, view2, pred1_512, pred2_512, save_dir, grad_coarseness=self.grad_coarseness, name="_512_pred1")
            save_render_images(color_512_pred2, None, mask2, save_dir, name="_512_pred2")
            save_render_images(color_256_pred1, None, mask1, save_dir, name="_256_pred1")
            save_render_images(color_256_pred2, None, mask2, save_dir, name="_256_pred2")
            save_render_images(color_128_pred1, None, mask1, save_dir, name="_128_pred1")
            save_render_images(color_128_pred2, None, mask2, save_dir, name="_128_pred2")
            ...
            with torch.no_grad():
                ...
                color_512, depth_512 = pl_module.decoder(batch, pred1_512, pred2_512, (h, w), fused_gaussians=self.grad_coarseness)
                color_256, depth_256 = pl_module.decoder(batch, pred1_256, pred2_256, (h, w), fused_gaussians=self.grad_coarseness)
                color_128, depth_128 = pl_module.decoder(batch, pred1_128, pred2_128, (h, w), fused_gaussians=self.grad_coarseness)
            ...
            log_batch_files(batch, color, depth, mask, view1, view2, pred1, pred2, save_dir, grad_coarseness=self.grad_coarseness, name="_512")
            save_render_images(color_256, None, mask, save_dir, name="_256")
            save_render_images(color_128, None, mask, save_dir, name="_128")
...
def save_as_ply(pred1, pred2, save_path, as_list=False, grad_coarseness=False):
    ...
    # means = pred2["means"].unsqueeze(0)  # Remove the batch dimension
    # covariances = pred2["covariances"].unsqueeze(0)  # Remove the batch dimension
    # harmonics = pred2["sh"].unsqueeze(0)[..., 0]  # Only use the first harmonic
    # opacities = pred2["opacities"].unsqueeze(0)  # Remove the batch dimension
...
@torch.no_grad()
def log_batch_files(batch, color, depth, mask, view1, view2, pred1, pred2, save_dir, should_save_3d=False, grad_coarseness=False, name=""):
    ...
    #     torchvision.utils.save_image(color[b, ...], os.path.join(save_dir, f"sample_{b}_rendered_color{coarseness_name}.jpg"))
    # if depth is not None:
    #     for b in range(min(color.shape[0], 4)):
    #         torchvision.utils.save_image(depth[b, :, None, ...], os.path.join(save_dir, f"sample_{b}_rendered_depth{coarseness_name}.jpg"), normalize=True)
    save_render_images(color, depth, mask, save_dir, name=name)
...
def save_render_images(color, depth, mask, save_dir, name):
    masked_predictions = color * mask[..., None, :, :]
    for b in range(min(color.shape[0], 4)):
        torchvision.utils.save_image(color[b, ...], os.path.join(save_dir, f"sample_{b}_rendered_color{name}.jpg"))
        torchvision.utils.save_image(masked_predictions[b], os.path.join(save_dir, f"sample_{b}_masked_rendered_color{name}.jpg"))

    if depth is not None:
        for b in range(min(color.shape[0], 4)):
            torchvision.utils.save_image(depth[b, :, None, ...], os.path.join(save_dir, f"sample_{b}_rendered_depth{name}.jpg"), normalize=True)
...
@torch.no_grad()
def log_batch_files_penalty_optimisation(batch, colors_combined, mask, view1, view2, pred1, pred2, coarseness1, coarseness2, save_dir, colors=None, should_save_3d=False, grad_coarseness=False, pl_module=None):
    '''Save all the relevant debug files for a batch
    color: (b, v, c, h, w) - the rendered color images for the batch
    mask: (b, v, h, w) - the loss mask for the batch
    '''
    (color_512, color_256, color_128, color_512_context, color_256_context, color_128_context) = colors
    os.makedirs(save_dir, exist_ok=True)
    ...
    # argmax_1 = torch.argmax(torch.softmax(coarseness1, dim=1), dim=1, keepdim=True)
    # argmax_2 = torch.argmax(torch.softmax(coarseness2, dim=1), dim=1, keepdim=True)
    argmax_1 = coarseness1
    argmax_2 = coarseness2
    # print(f"argmax shape: {argmax.shape}, argmax min: {torch.min(argmax)}, argmax max: {torch.max(argmax)}, mean: {torch.mean(argmax.float())}")
    coarseness_pred_one_hot_1.scatter_(1, argmax_1, 1.0) # (v, 3, h, w)
    coarseness_pred_one_hot_2.scatter_(1, argmax_2, 1.0) # (v, 3, h, w)
    coarseness_pred_one_hot = torch.cat((coarseness_pred_one_hot_1, coarseness_pred_one_hot_2), dim=0) # (b, v, 3, h, w)
    ...
    depth2_mask = (view2['depthmap'] > 1e-6)
    depth_masks = torch.stack([depth1_mask, depth2_mask], dim=1) # (b, v, h, w) boolean mask indicating valid pixels
    coarseness_gt_render = pl_module.get_gt_coarseness(batch, colors[3:], mask=depth_masks, context_view=True)
    coarseness_gt_render = torch.nn.functional.one_hot(coarseness_gt_render, num_classes=3)
    # coarseness_gt_render = coarseness_gt_render - coarseness_gt_render.min() / (coarseness_gt_render.max() - coarseness_gt_render.min())
    coarseness_gt_render = coarseness_gt_render.float()
    coarseness_gt_render = einops.rearrange(coarseness_gt_render, "v h w c -> v c h w")
    # mask_repeated = mask.unsqueeze(2).repeat(1, 1, 3, 1, 1)
    ...
    # print(f"coarseness_gt_render.shape: {coarseness_gt_render}")
    # Normalize the color tensor such that the channel with the greatest value becomes 1.0 and others become 0.0
    color_one_hot = torch.zeros_like(colors_combined)
    zero_mask = (colors_combined == 0.0)
    max_indices = torch.argmax(colors_combined, dim=2, keepdim=True)  # Find the index of the max value along the channel dimension
    color_one_hot.scatter_(2, max_indices, 1.0)  # Set the max channel to 1.0 and others to 0.0
    color_one_hot[zero_mask] = 0.0  # Set the zero values to 0.0 in the one-hot encoded tensor
    # print(f"zero_mask.shape: {zero_mask.shape}, coarseness_gt_render.shape: {coarseness_gt_render.shape}")
    # print(f"mask.shape: {mask.shape}, coarseness_gt_render.min(): {coarseness_gt_render.min()}, coarseness_gt_render.max(): {coarseness_gt_render.max()}")
    ...
    for b in range(min(context_images.shape[0], 4)):
        ...
        # torchvision.utils.save_image(coarseness_pred_one_hot, os.path.join(save_dir, f"sample_{b}_coarseness_render_gt.jpg"), normalize=True)
        torchvision.utils.save_image(coarseness_gt, os.path.join(save_dir, f"sample_{b}_coarseness_gradient_mask.jpg"), normalize=True)
        # torchvision.utils.save_image(crossentropy_loss_1, os.path.join(save_dir, f"sample_{b}_crossentropyloss.jpg"), normalize=True)
        torchvision.utils.save_image(coarseness_gt_render, os.path.join(save_dir, f"sample_{b}_coarseness_gt_render.jpg"), normalize=True)
        if "mask_highres" in pred1.keys():
            torchvision.utils.save_image(context_images_lod_masked[b].float(), os.path.join(save_dir, f"sample_{b}_LoD_mask_context.jpg"), normalize=True)
    ...
    # Save the rendered images and depths
    for b in range(min(colors_combined.shape[0], 4)):
        torchvision.utils.save_image(colors_combined[b, ...], os.path.join(save_dir, f"sample_{b}_rendered_color.jpg"))
        torchvision.utils.save_image(color_one_hot[b, ...], os.path.join(save_dir, f"sample_{b}_rendered_color_one_hot.jpg"))
    ...
    target_original_images = torch.stack([view["original_img"] for view in batch["target"]], dim=1)
    masked_target_original_images = target_original_images * mask[..., None, :, :]
    masked_predictions = colors_combined * mask[..., None, :, :]
    for b in range(min(target_original_images.shape[0], 4)):
        torchvision.utils.save_image(masked_target_original_images[b], os.path.join(save_dir, f"sample_{b}_masked_original_img_target.jpg"))
        torchvision.utils.save_image(masked_predictions[b], os.path.join(save_dir, f"sample_{b}_masked_rendered_color.jpg"))